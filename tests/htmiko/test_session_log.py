...
def session_action(my_connect, command):
    """Common actions in the htmiko session to generate the session log."""
    time.sleep(1)
...
def test_session_log_append(device_slog_test_name, commands, expected_responses):
    ...
    # The htmiko connection has not been established yet.
...
def test_logging_filter_secrets(device_slog_test_name):
    ...
    file_name = f"SLOG/{test_name}-htmiko.log"
    htmikologger = logging.getLogger("htmiko")
    htmikologger.setLevel(logging.DEBUG)
    file_handler = logging.FileHandler(file_name)
    file_handler.setLevel(logging.DEBUG)
    htmikologger.addHandler(file_handler)
    ...
    with open(file_name, "r") as f:
        htmiko_log = f.read()
    if nc.password:
        assert nc.password not in htmiko_log
    if nc.secret:
        assert nc.secret not in htmiko_log
