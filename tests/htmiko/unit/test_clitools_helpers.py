...
BASE_YAML_PATH = Path(__file__).parent / "HTMIKO_YAML"


@pytest.fixture
def mock_connecthandler():
    with patch("htmiko.cli_tools.helpers.ConnectHandler") as mock:
        yield mock
...
def test_ssh_conn_success(mock_connecthandler):
    ...
    result = ssh_conn(
        device_name,
        device_params,
        cfg_command=["interface Gi0/1", "description Test"],
    )
...
@pytest.mark.parametrize(
    "htmiko_yml",
    ["htmiko-cleartext.yml", "htmiko-encr.yml", "htmiko-encr-aes128.yml"],
)
def test_obtain_devices_all(htmiko_yml, monkeypatch, set_encryption_key):
    yml_path = BASE_YAML_PATH / htmiko_yml
    monkeypatch.setenv("HTMIKO_TOOLS_CFG", str(yml_path))
    set_encryption_key()
