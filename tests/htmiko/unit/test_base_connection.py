...
def test_timeout_exceeded():
    """Raise HTMikoTimeoutException if waiting too much"""
    connection = FakeBaseConnection(session_timeout=10)
    start = time.time() - 11
    try:
        connection._timeout_exceeded(start)
    except HTMikoTimeoutException as exc:
        assert isinstance(exc, HTMikoTimeoutException)
        return
...
def test_timeout_not_exceeded():
    """Do not raise HTMikoTimeoutException if not waiting too much"""
    connection = FakeBaseConnection(session_timeout=10)
    start = time.time()
...
def test_unlocking_no_lock():
    ...
    connection._unlock_htmiko_session()
    assert not connection._session_locker.locked()
...
def test_locking_unlocking():
    ...
    connection = FakeBaseConnection()
    assert not connection._session_locker.locked()
    connection._lock_htmiko_session()
