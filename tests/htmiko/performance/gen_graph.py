...
def read_csv(device):
    csv_file = "htmiko_performance.csv"
    entries = []
...
def filter_versions(entries):
    ...
    entries = filter(lambda x: x["htmiko_version"] in last_versions, entries)
    return sorted(entries, key=lambda x: x["htmiko_version"])
...
def generate_graph(device_name: str, params: Dict) -> None:
    ...
    outfile = f"htmiko_{device_type}.svg"
    entries = read_csv(device)
    # entries = filter_versions(entries)

    # Create relevant lists
    htmiko_versions = [v["htmiko_version"] for v in entries]
    ...
    pprint(entries)
    print(htmiko_versions)
    print(connect)

    # Graph It
    line_chart = pygal.Line(include_x_axis=True)
    line_chart.title = title
    line_chart.x_labels = htmiko_versions
