...
def read_devices():
    ...
    try:
        ...
        print(f"Unable to open {f_name}: {exc}")
        return {}
...
def write_csv(device_name, results):
    results_file = "htmiko_scrapli_performance.csv"
...
# from datetime import datetime
#
# from htmiko import ConnectHandler, __version__
#
#
#
...
def send_command_simple(driver, device):
    ScrapliClass = globals()[driver]
    with ScrapliClass(**device) as conn:
        platform = htmiko_scrapli_platform[str(driver)]
