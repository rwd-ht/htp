...
def read_csv(device):
    csv_file = "htmiko_scrapli_performance.csv"
    entries = []
...
def generate_graph(device_name: str, params: Dict) -> None:
    ...
    device_type = htmiko_scrapli_platform[str(driver)]

    if device_name == "cisco1":
        device_type = "cisco_ios"
    outfile = f"htmiko_scrapli_{device_type}.svg"
    entries = read_csv(device)
    # entries = filter_versions(entries)

    # Create relevant lists
    htmiko_version = "4.1.0"
    htmiko_results = []
    scrapli_version = "2022.1.30.post1"
    scrapli_results = []
    for v in entries:
        if v["version"] == htmiko_version:
            htmiko_results.append(convert_time(v["connect"]))
            htmiko_results.append(convert_time(v["send_command_simple"]))
            htmiko_results.append(convert_time(v["send_config_simple"]))
            htmiko_results.append(convert_time(v["send_config_large_acl"]))
    ...
    line_chart.add(f"HTMiko {htmiko_version}", htmiko_results)
    line_chart.add(f"Scrapli {scrapli_version}", scrapli_results)
    dir_path = Path.cwd() / "graphs_htmiko_scrapli"
...
# HTMiko-Scrapli Performance Comparison
# HTMiko Version: {{htmiko_version}}
...
def generate_report():
    ...
    graphs_path = Path.cwd() / "graphs_htmiko_scrapli"
    graph_files = [
        "graphs_htmiko_scrapli/" + item.name for item in graphs_path.iterdir()
    ]
    report_file = Path.cwd() / "performance_htmiko_scrapli.md"
    with report_file.open("w") as out_file:
        j2_vars = {}
        j2_vars["graphs"] = graph_files
        j2_vars["htmiko_version"] = "4.1.0"
