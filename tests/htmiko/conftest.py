...
from htmiko import FileTransfer
from htmiko import InLineTransfer
from htmiko import SSHDetect
...
def net_connect(request):
    ...
    Return the htmiko connection object
    """
...
def device_slog(request):
    ...
    Return the htmiko device (not connected)
    """
    device_under_test = request.config.getoption("test_device")
    test_devices = parse_yaml(PWD + "/etc/test_devices.yml")
    device = test_devices[device_under_test]
