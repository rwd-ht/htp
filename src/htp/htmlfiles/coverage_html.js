...
function sortColumn(th) {
    ...
    [...th.parentElement.cells].forEach((header) =>
    ...
    Array.from(th.closest('table').querySelectorAll('tbody tr'))
        .sort(
            (rowA, rowB) =>
                rowComparator(rowA, rowB, column) *
...
coverage.assign_shortkeys = function () {
    document.querySelectorAll('[data-shortcut]').forEach((element) => {
        document.addEventListener(
            element.dataset.shortcut === 'f' ? 'keyup' : 'keydown',
            (event) => {
                if (event.target.tagName.toLowerCase() === 'input') {
                    return; // ignore keypress from search filter
                }
                if (event.target.tagName.toLowerCase() === 'textarea') {
                    return; // ignore keypress from fix window
                }
...
coverage.wire_up_filter = function () {
    ...
    const ratio_columns = Array.from(footer.cells).map((cell) =>
        Boolean(cell.dataset.ratio),
    );

    // Observe filter keyevents.
    const filter_handler = (event) => {
        // Keep running total of each metric, first index contains number of shown rows
        const totals = ratio_columns.map((is_ratio) =>
            is_ratio ? { numer: 0, denom: 0 } : 0,
        );
        ...
        table_body_rows.forEach((row) => {
            ...
            if (show && hide100) {
                const [numer, denom] =
                    row.cells[row.cells.length - 1].dataset.ratio.split(' ');
                show = numer !== denom;
            }
    ...
    document
        ...
        .addEventListener('input', debounce(filter_handler));
    document
        .getElementById('hide100')
        .addEventListener('input', debounce(filter_handler));
...
coverage.wire_up_sorting = function () {
    document
        .querySelectorAll('[data-sortable] th[aria-sort]')
        .forEach((th) =>
            th.addEventListener('click', (e) => sortColumn(e.target)),
        );
    ...
    if (sorted_by_region) {
        ({ by_region, region_direction } = JSON.parse(sorted_by_region));
    }
...
coverage.pyfile_ready = function () {
    ...
    on_click('.button_toggle_oval', coverage.toggle_lines);
    on_click('.button_toggle_oinv', coverage.toggle_lines);
    on_click('.button_toggle_ount', coverage.toggle_lines);
    on_click('.button_toggle_ofix', coverage.toggle_lines);
    ...
    document
        .querySelectorAll('[id^=ctxs]')
        .forEach((cbox) =>
            cbox.addEventListener('click', coverage.expand_contexts),
        );

    // Rebuild scroll markers when the window height changes.
    window.addEventListener('resize', coverage.build_scroll_markers);

    const fixesWindow = document.getElementById('fixesWindow');
    const openFixes = document.getElementById('openFixes');
    const closeFixes = document.getElementById('closeFixes');
    openFixes.addEventListener('click', () => {
        fixesWindow.showModal();
    });
...
coverage.toggle_lines = function (event) {
    ...
    try {
        localStorage.setItem(
            ...
            JSON.stringify(coverage.filters),
        );
    } catch (err) {}
};
...
coverage.set_line_visibilty = function (category, should_show) {
    ...
    if (btn) {
        if (should_show) {
            document
                .querySelectorAll('#source .' + category)
                .forEach((e) => e.classList.add(cls));
...
coverage.selection_ends_on_screen = function () {
    ...
    return (checkVisible(begin) ? 1 : 0) + (checkVisible(end) ? 1 : 0);
};
...
coverage.show_selection = function () {
    ...
    document
        .querySelectorAll('#source .highlight')
...
coverage.build_scroll_markers = function () {
    ...
    const line_height = Math.min(
        Math.max(3, window.innerHeight / coverage.lines_len),
        10,
    );
    ...
    document
        ...
        .forEach((element) => {
            ...
            const line_number = parseInt(
                element.querySelector('.n a').id.substr(1),
            );
