...
from htmiko.utilities import display_inventory
from htmiko.utilities import load_devices
...
def common_args(parser):
    ...
    parser.add_argument(
        "--list-devices",
        help="List devices from inventory",
        action="store_true",
    )
    parser.add_argument(
        "--display-runtime",
        help="Display program runtime",
        action="store_true",
    )
    parser.add_argument(
        "--hide-failed",
        help="Hide failed devices",
        action="store_true",
    )
    parser.add_argument(
        "--json",
...
def show_args(parser):
    """Add arguments specific to htmiko_show.py."""
    parser.add_argument(
        "devices",
...
def cfg_args(parser):
    """Add arguments specific to htmiko_cfg.py."""
    parser.add_argument(
        "devices",
        help="Device or group to connect to",
        action="store",
...
def grep_args(parser):
    """Add arguments specific to htmiko_grep.py."""
    parser.add_argument(
        "pattern",
        nargs="?",
...
def parse_arguments(args, command):
    ...
    if command == "htmiko-cfg":
        description = "Execute configurations command"
        addl_args = cfg_args
    elif command == "htmiko-show":
        description = "Execute show command (defaults to 'show run')"
        addl_args = show_args
    elif command == "htmiko-grep":
        description = "Grep pattern search on output (defaults to 'show run')"
        addl_args = grep_args
    else:
        raise ValueError(f"Unknown cli-tool: {command}")
    ...
    if not cli_args.list_devices and not cli_args.version:
        ...
        elif command == "htmiko-cfg" and not cli_args.cmd and not cli_args.infile:
            parser.error("No configuration commands provided.")
        elif command == "htmiko-grep" and not cli_args.pattern:
