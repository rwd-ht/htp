...
"""
Base connection class for htmiko
...
from htmiko.channel import SerialChannel
from htmiko.channel import SSHChannel
from htmiko.channel import TelnetChannel
...
from htmiko.htmiko_globals import BACKSPACE_CHAR
from htmiko.session_log import SessionLog
from htmiko.utilities import check_serial_port
...
class BaseConnection:
    ...
    def __init__(
        ...
        :param global_delay_factor: Multiplication factor affecting delays (default: 1).
        ...
        :param auto_connect: Control whether the connection is automatically established as
                part of the object creation (default: True).

        :param disable_lf_normalization: Disable linefeed normalization behavior
                (default: False)
        """
    ...
    def _timeout_exceeded(self, start: float, msg: str = "Timeout exceeded!") -> bool:
        """Raise HTMikoTimeoutException if waiting too much in the serving queue.
    ...
    def _lock_htmiko_session(self, start: float | None = None) -> bool:
        ...
        while not self._session_locker.acquire(False) and not self._timeout_exceeded(
            start,
            "The htmiko channel is not available!",
        ):
            time.sleep(0.1)
        return True

    def _unlock_htmiko_session(self) -> None:
        """
...
"""
        ...
        msg = f"""Pattern not detected: {pattern!r} in output"""
    ...
    def read_channel_timing(
        ...
        :param read_timeout: Absolute timer for how long the loop should keep reading data on
            ...
            (i.e. the loop will keep reading indefinitely until there is no new data and last_read
            passes).
        ...
        while (time.time() - start_time < read_timeout) or (not read_timeout):
            ...
            msg = f"read_channel_timing's absolute timer of {read_timeout} expired."
            raise ReadTimeout(msg)
        return channel_data
    ...
    def telnet_login(
        ...
        while i <= max_loops:
            try:
                ...
                raise HTMikoAuthenticationException(msg)
    ...
    def _try_session_preparation(self, force_data: bool = True) -> None:
        """
        In case of an exception happening during `session_preparation()` this should
        ...
        """
        try:
            # there needs to be data for session_preparation to work.
            if force_data:
    ...
    def establish_connection(self, width: int = 511, height: int = 1000) -> None:
        ...
        Timeout will generate a HTMikoTimeoutException
        Authentication failure will generate a HTMikoAuthenticationException
        ...
        if self.protocol == "telnet":
            ...
            try:
                ...
                msg = f"TCP connection to device failed: {self.device_type} {self.host}:{self.port}"
                ...
                self.paramiko_cleanup()
                msg = f"Authentication to device failed: {self.device_type} {self.host}:{self.port}"
                ...
                if "No existing session" in str(e):
                    ...
                    msg = f"A paramiko SSHException occurred during connection creation: {e}"
                    raise HTMikoTimeoutException(msg)
    ...
    def _test_channel_read(self, count: int = 40, pattern: str = "") -> str:
        ...
        raise HTMikoTimeoutException("Timed out waiting for data")
