...
"""
This module is used to auto-detect the type of a device in order to automatically create a connection.

This avoids hard coding the 'device_type' when using the ConnectHandler factory function

Example:
------------------
from htmiko.snmp_autodetect import SNMPDetect
...
htmiko requirements. So installation of pysnmp might be required.
"""
...
from htmiko.ssh_dispatcher import CLASS_MAPPER
...
# Ensure all SNMP device types are supported
SNMP_MAPPER = {}
...
class SNMPDetect(object):
    ...
    def autodetect(self) -> Optional[str]:
        ...
        is returned is directly matching the name in *htmiko.ssh_dispatcher.CLASS_MAPPER_BASE*
        dict.
