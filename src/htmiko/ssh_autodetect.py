...
"""
The ssh_autodetect module is used to auto-detect the htmiko device_type to use to further initiate
...
The **SSHDetect** class is instantiated using the same parameters as a standard
connection (see the *htmiko.ssh_dispatacher.ConnectHandler* function). The only acceptable value
...
>>> from htmiko.ssh_autodetect import SSHDetect
>>> from htmiko.ssh_dispatcher import ConnectHandler
...
class SSHDetect(object):
    ...
    ----------
    *args : list
        The same *args that you might provide to the htmiko.ssh_dispatcher.ConnectHandler.
    *kwargs : dict
        The same *kwargs that you might provide to the htmiko.ssh_dispatcher.ConnectHandler.

    Attributes
    ----------
    connection : htmiko.terminal_server.TerminalServerSSH
